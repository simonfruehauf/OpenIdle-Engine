...
const ItemTooltip: React.FC<{ item: ItemConfig; rect: DOMRect | null }> = ({ item, rect }) => {
    ...
    // Helper to get friendly name from ID
    ...
    const renderEffect = (e: any, idx: number) => {
        ...
        if (e.type === 'modify_max_resource_flat') {
            return <div key={idx} className="text-blue-700">{e.amount > 0 ? '+' : ''}{e.amount} Max {getName(e.resourceId)}</div>;
        }
        if (e.type === 'modify_max_resource_pct') {
            return <div key={idx} className="text-blue-700">{e.amount > 0 ? '+' : ''}{Math.round(e.amount * 100)}% Max {getName(e.resourceId)}</div>;
        }
        if (e.type === 'modify_passive_gen') {
            return <div key={idx} className="text-green-700">{e.amount > 0 ? '+' : ''}{e.amount}/s {getName(e.resourceId)}</div>;
        }
        if (e.type === 'modify_yield_pct') {
            ...
            return <div key={idx} className="text-purple-700">{e.amount > 0 ? '+' : ''}{Math.round(e.amount * 100)}% Yield {target}</div>;
        }
        if (e.type === 'modify_yield_flat') {
            const target = e.taskId ? `(${getName(e.taskId)})` : e.actionId ? `(${getName(e.actionId)})` : e.resourceId ? `to ${getName(e.resourceId)}` : '';
            return <div key={idx} className="text-purple-700">{e.amount > 0 ? '+' : ''}{e.amount} Yield {target}</div>;
        }
        // Legacy support
        if (e.type === 'modify_task_yield_pct') {
            return <div key={idx} className="text-purple-700">{e.amount > 0 ? '+' : ''}{Math.round(e.amount * 100)}% Yield ({getName(e.taskId)})</div>;
        }
        return null;
    };
